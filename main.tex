...
\begin{comment}
\paragraph{Metody głosowania}

W obrębie komitetów ekspozerów wprowadzone zostało pięć propozycji głosowania.

\begin{itemize}
		\item \textbf{Brak miary} --- każdy ekspozer ma taki sam wpływ na decyzję komitetu.

\begin{equation}
	\Pi_\textrm{\textsc{a}}(x_k) = \mathop{argmax}\limits_{i \in \mathcal{M}}\Bigg(\sum_{l = 1}^N pix^{(l)}\Big(\Psi_l(x_k)\Big)\Bigg)
\end{equation}

	\item \textbf{Pojedyncza miara pewności dla klasy} --- dla każdego \emph{ekspozera} w komitecie wyliczamy uśredniony wektor wsparć dla każdej z klas. Wektor wsparcia komitetu to suma tych wektorów.

\begin{equation}
	\Pi_\textrm{\textsc{b}}(x_k) = \mathop{argmax}\limits_{i \in \mathcal{M}}\Bigg(\sum_{l = 1}^N\theta^{(l)}\cdot pix^{(l)}\Big(fin(x_k)\Big)\Bigg)
\end{equation}

	\item \textbf{Pojedyncza miara pewności dla ekspozera} --- The linear combination of the  $\beta_i$ with $\theta_i$ weights our voting vector ($V$).
